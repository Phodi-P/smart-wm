...
def processImage(path, progress_list, settings):
        ...
        logo_B = Visibility.getBrightness(logo)
        result = Paste.pasteLogo(path, io_path, logo, logo_B, settings.preferedPositions, settings.pickTheMostVisiblePosition,
    ...
    while(True):
        ...
        if answer["menu"] == 'settings':
            ...
            if answer_position_local["defaultPos"] == True:
                answer_position = {'positions': [(50,100,'Bot Middle'),(0,100,'Bot Left'),(100,100,'Bot Right')]}
            else:
                while(True):
